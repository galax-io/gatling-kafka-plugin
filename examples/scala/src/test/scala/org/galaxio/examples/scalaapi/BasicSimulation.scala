...
class BasicSimulation extends Simulation {
  ...
  def scn(id: String): ScenarioBuilder = scenario(s"Basic$id")
    ...
    .exec(
      kafka("ReqRep").requestReply
        .requestTopic("ex.scala.basic.t")
        .replyTopic("ex.scala.basic.t")
  ...
  setUp(
    scn("A").inject(atOnceUsers(2)),
    scn("B").inject(atOnceUsers(1)),
  ).protocols(kafkaProtocolC)
    .maxDuration(120.seconds)
    .assertions(global.allRequests.count.is(6), global.successfulRequests.percent.is(100))
